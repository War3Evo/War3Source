...
{
	EnableFullDownloadsModuleCvar = CreateConVar("war3_downloads_full_download", "1","1 to enable module", FCVAR_PLUGIN);
	
	EnableDownloadsModuleCvar = CreateConVar("war3_downloads_enabled", "1","1 to enable module", FCVAR_PLUGIN);
	EnablePRECACHEDownloadsCvar = CreateConVar("war3_downloads_precache_enabled", "0","set 1 to enable downloads of Priority TOP", FCVAR_PLUGIN);
	EnableTopDownloadsCvar = CreateConVar("war3_downloads_top_enabled", "1","set 1 to enable downloads of Priority TOP", FCVAR_PLUGIN);
	EnableHighDownloadsCvar = CreateConVar("war3_downloads_high_enabled", "1","set 1 to enable downloads of Priority TOP", FCVAR_PLUGIN);
	EnableMediumDownloadsCvar = CreateConVar("war3_downloads_medium_enabled", "1","set 1 to enable downloads of Priority TOP", FCVAR_PLUGIN);
	EnableLowDownloadsCvar = CreateConVar("war3_downloads_low_enabled", "1","set 1 to enable downloads of Priority TOP", FCVAR_PLUGIN);
	EnableBottomDownloadsCvar = CreateConVar("war3_downloads_bottom_enabled", "1","set 1 to enable downloads of Priority TOP", FCVAR_PLUGIN);

	imaxdownloadsCvar = CreateConVar("war3_downloads_max", "2","max allowed downloads per player (max * maxclients)", FCVAR_PLUGIN);
	ihighdownloadsCvar = CreateConVar("war3_downloads_high", "0.50","percentage 0.50 is 50 percent", FCVAR_PLUGIN);
	imediumdownloadsCvar = CreateConVar("war3_downloads_medium", "0.25","percentage 0.25 is 25 percent", FCVAR_PLUGIN);
	ilowdownloadsCvar = CreateConVar("war3_downloads_low", "0.10","percentage 0.10 is 10 percent", FCVAR_PLUGIN);
	ibottomdownloadsCvar = CreateConVar("war3_downloads_bottom", "0.05","percentage 0.05 is 5 percent", FCVAR_PLUGIN);
	Log_TOP_prioirtyCvar = CreateConVar("war3_downloads_log_top", "0","set 1 to enable", FCVAR_PLUGIN);
	Log_HIGH_prioirtyCvar = CreateConVar("war3_downloads_log_high", "0","set 1 to enable", FCVAR_PLUGIN);
	Log_MEDIUM_prioirtyCvar = CreateConVar("war3_downloads_log_medium", "0","set 1 to enable", FCVAR_PLUGIN);
	Log_LOW_prioirtyCvar = CreateConVar("war3_downloads_log_low", "0","set 1 to enable", FCVAR_PLUGIN);
	Log_BOTTOM_prioirtyCvar = CreateConVar("war3_downloads_log_bottom", "0","set 1 to enable", FCVAR_PLUGIN);
 ...
 */
bool:War3_AddSoundFiles(const String:sound[], bool:precache = true, bool:preload = true)
{
...
bool:AddSoundFiles(const String:sound[],iSoundIndex,iMaxDownloadsCount,iCurrentCount)
{
					...
					if(FileExists(path) && GetArrayCell(g_hStockSound, iSoundIndex)>0)
					{
						...
						if(!PrecacheSound(SoundModify, false))
						{
							Format(StringDetail,sizeof(StringDetail),"War3Source_Engine_Download_Control: (PrecacheSound) Sound file \"%s\" not found", SoundModify);
							LogDownloads(StringDetail);
						}
						else
						{
							...
							{
								Format(StringDetail,sizeof(StringDetail),"**PRECACHED** %s",SoundModify);
								LogDownloads(StringDetail);
							}
						}
					}
