...
new bool:g_RestoreItemsFromDeath_playerOwnsItem[MAXPLAYERSCUSTOM][MAXITEMS+1];
    ...
    if (ValidPlayer(client))
        {
            ...
            {
                if(playerOwnsItem[client][i]==true||g_RestoreItemsFromDeath_playerOwnsItem[client][i]==true)
                {
                    W3SetVar(EventArg1,i);
                    if(W3Denyable(DN_CanBuyItem1,client)==false)
                    {
                        g_RestoreItemsFromDeath_playerOwnsItem[client][i]=false;
            ...
            if(payforit==true)
            {
                ...
                for(new i;i<=ItemsLoaded;i++)
                {
                    if((playerOwnsItem[client][i]==g_RestoreItemsFromDeath_playerOwnsItem[client][i]) && (playerOwnsItem[client][i]==true))
                    {
                        checkit=true;
                        counter++;
                    }
                ...
                for(new i;i<=ItemsLoaded;i++)
                {
                    if(War3_GetOwnsItem(client,i) && !g_RestoreItemsFromDeath_playerOwnsItem[client][i])
                    {
                ...
                for(new i;i<=ItemsLoaded;i++)
                {
                    ...
                    if(SkipBuying == false)
                    {
                        if(g_RestoreItemsFromDeath_playerOwnsItem[client][i])
                        {
                            if(!War3_GetOwnsItem(client,i))
                            {
                                //How much does the item cost?
                                War3_Temp_ItemCost = W3GetItemCost(i);
            ...
            for(new i;i<=ItemsLoaded;i++)
            {
                ...
                if(g_RestoreItemsFromDeath_playerOwnsItem[client][i]==true)
                {
                    War3_SetOwnsItem(client,i,true);
                    W3GetItemName(i,itemName,64);
                ...
                if(g_RestoreItemsFromDeath_playerOwnsItem[client][i]==false && War3_GetOwnsItem(client,i))
                {
                    War3_SetOwnsItem(client,i,false);
                    W3GetItemName(i,itemName,64);
                    War3_ChatMessage(client,"{red}<<buyprevious>>{default}Item Discarded: {green}%s{default} (You didn't own this item on death).",itemName);
...
public OnWar3Event(W3EVENT:event,client){
    ...
    if(event==OnDeathPre)
    {
        ...
        if(GetClientItemsOwned(client)>0)
        {
            ...
            {
                    g_RestoreItemsFromDeath_playerOwnsItem[client][i2]=playerOwnsItem[client][i2];
            }
        }
    }
}
